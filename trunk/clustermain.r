...
layerstruc <- c(2,3); 
inputdata <- "/home/jaeger/B/SWF/logistic.r";
...
# TODO find R format for date+time without space
# DONE! PAOLO 23-05-2012
outputbase <- paste("clusterout-",sub(" ","_",Sys.time()),sep="");
outputfile <- paste(outputbase,"_iter_",itcount,sep="");

savecolsxml(datatable,(numattributes+1):(numattributes+numlayers), outputfile);
...
outputfile <- paste(outputbase,"_iter_",itcount,"_",likelihood,sep="");
